...
int readlist(struct process list[])
{
	...
	for(i=0;i<n;i++)
	{
		...
		printf("\n Enter %d Priority:\t",i+1);
		scanf(" %d",&list[i].priority);
		//list[i].priority = rand()%n;
		list[i].WT=list[i].TAT=0;
	}
	printf("PID\tBT\tAT\tPrio\n");
	for(i=0;i<n;i++)
	{
		printf("\n%d\t%d\t%d\t%d\n",list[i].pid,list[i].BT,list[i].AT,list[i].priority);
	}
	printf("\n");
	ps=n;
	return ps;
...
int GanttChart(struct process list[],struct process Q[])
{
	...
	while(listn>0 || qn>0)
	{
		...
		Q[0].BT--;
		Q[0].WT++;
		//printf("\n %d\t%d\t%d",Q[0].pid,Q[0].BT,Q[0].WT); //Can be used to print the gantt chart at each clock (print variables to be changed)
		if(Q[0].BT==0)
		{
			Q[0].TAT= ctr-Q[0].AT;
